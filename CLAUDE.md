...
- Points math (`resolveCostsForUnitIndex`, `sumDetachmentPoints`, model-count resolution, rule selection caps) lives in `src/core/utils/roster.ts`; weapon-option/loadout parsing is in `src/core/utils/weaponOptions.ts`. Points are never cached on a `RosterEntry`/`RosterList` (no `pointsCost`, `wargearSurcharge`, or `totalPoints` fields) — `RosterEntry` only stores the player's choices (`modelCount`, `costDescription`, `wargearSelections`, `weaponOptionSelections`, `enhancementId`). Every points figure is resolved fresh from the current `pointsCostMap`/`wargearCostMap`/`enhancements` at read time via `resolveEntryCostVariant` / `resolveEntryBaseCost` / `resolveEntryWargearSurcharge` / `resolveEntryEnhancementCost` / `resolveEntryPoints` (base + wargear, what the roster editor shows per unit) / `resolveEntryTotalPoints` (+ enhancement, for export text) / `resolveRosterTotalPoints` (whole-roster grand total) — all in `src/core/utils/roster.ts`. This means a correction to a datasheet's points in the JSON data is reflected on every saved roster immediately, with nothing to re-save; it also shrinks what's persisted (backend `PUT /api/rosters/:id`, QR payload). `costDescription` (the tier-stripped `PointsCost.description` the player picked) matters because `modelCount` alone doesn't always identify a unique cost variant — some datasheets have multiple pricing options that parse to the same leading model count but price differently (e.g. Gretchin's "20 Gretchin" vs "1 Runtherd, 20 Gretchin"); `resolveEntryCostVariant` prefers matching it and only falls back to a `modelCount`-only guess for entries saved before this field existed.
