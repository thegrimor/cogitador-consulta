...
- **Default-active own abilities** (`MathhammerPage.tsx`): a selected unit's or attached
  character's own (non-`isOption`) ability rules — `ModifierRule.datasheetId`/`leaderDatasheetId`
  scoped, i.e. what `deriveModifierRules` derives from `selectedUnit.abilities`/
  ...
  alternatives).
- **Default-active Feel No Pain + Stealth, regardless of source**: two more defender-only
  defaults, seeded at the same point as the own-ability one above (only when there's no saved
  per-unit state). `feelNoPainDefaultIds()` turns on *any* currently-applicable rule (already
  filtered through `isRuleApplicable`, so a `requiresTargetKeyword`-gated one only counts when
  the selected unit actually qualifies) whose effect sets `feelNoPainThreshold` — unlike the
  own-ability default above, this isn't restricted to `datasheetId`/`leaderDatasheetId`-scoped
  rules, since a persistent FNP grant just as often comes from a Detachment Ability or Army Rule
  (e.g. Imperial Knights' Freeblade Company detachment ability "Knights of Legend": flat FNP 6+,
  or Adeptus Mechanicus' Lords of the Forge detachment ability "War-Form Mantles": FNP 4+ gated
  on `requiresTargetKeyword: 'tech-priest'`) as from a plain datasheet ability — still excludes
  `isStratagem`/`isOption` rules, same reasoning as above. Separately, `hasAbilityNamed(unit,
  'Stealth')` checks whether the selected defender's own datasheet carries a literal "Stealth"
  ability, and if so defaults on the universal `unit-stealth` core rule
  (`gameData.coreCombatEffects`, `hitMod: -1`/`bsMod: 1`-style depending on data version,
  `target: 'defender'`) for it — Stealth can't be handled by `ownAbilityIds()` at all, because the
  per-datasheet "Stealth" ability is stored as a bare `{id, name, description, type: 'Core'}` stub
  with **no** `effect` of its own (every faction that has it); the actual toggleable mechanic only
  exists once, as that one universal core rule, decoupled from any specific datasheet, so
  `deriveModifierRules` has no way to know which unit it's "for" without this explicit
  ability-name check. This is unlike the generic "Cobertura" universal rule, which stays a manual
  toggle since it depends on battlefield terrain, not a fixed datasheet property. The
  `leftRuleCtx`/`rightRuleCtx`/`applicableLeftRules`/`applicableRightRules` computation (used both
  by the final `resolveModifiers` calls and by this default-seeding logic) now sits right after
  the `attackerActiveIds`/`defenderActiveIds` memos — earlier than it actually needs to be for the
  render itself — specifically so the restore-on-select logic further down can read
  `applicableRightRules` too; don't move it back below the restore blocks without re-threading
  that dependency.
- **Sticky mobile tab bar**: on the `md:hidden` mobile layout, the Atacante/Resultado/Defensor tab
  ...
  while scrolling a long weapons/abilities list in whichever tab is open, instead of scrolling out
  of view and forcing a scroll back to the top to switch tabs. Desktop's 3-column layout is
  unchanged — only the middle results column has ever had its own `sticky`/bounded-height wrapper.
